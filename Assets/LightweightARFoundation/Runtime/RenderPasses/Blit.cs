...
namespace FreakshowStudio.LightweightARFoundation.Runtime.RenderPasses
{
    public class Blit : ScriptableRendererFeature
    {
        ...
        {
            var passIndex = settings.blitMaterial != null
                ? settings.blitMaterial.passCount - 1
                : 1;
            ...
            ref RenderingData renderingData)
        {
            ...
            if (settings.blitMaterial == null)
            {
                Debug.LogWarningFormat(
                    "Missing Blit Material. {0} blit pass will not execute. " +
                    "Check for missing reference in the assigned renderer.",
                    GetType().Name);
